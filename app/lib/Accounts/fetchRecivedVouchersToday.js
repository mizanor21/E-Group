 ...
 */
export const getTodayCreatedReceivedVouchersFromAllYears = async () => {
  ...
  // Filter only voucher collections based on naming pattern (assuming they follow "recived_vouchers_YYYY" pattern)
  const voucherCollections = collections
    .filter(collection => /^recived_vouchers_\d{4}$/.test(collection.name))
    .map(collection => {
      // Extract year from collection name
      const yearMatch = collection.name.match(/^recived_vouchers_(\d{4})$/);
    ...
    const batchResults = await Promise.all(
      yearBatch.map(async (year) => {
        try {
          ...
          console.warn(`⚠️ Error fetching from recived_vouchers_${year}:`, error.message);
          return [];
        }
      })
    );
