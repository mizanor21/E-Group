...
const EditPaymentVoucherModal = ({ voucher, onClose, onUpdate }) => {
  const { 
    control,
    register,
    handleSubmit,
  ...
  const onSubmit = async (data) => {
    try {
      const response = await fetch(`/api/payment-vouchers/${voucher.date.split('-')[0]}/${voucher._id}`, {
        method: 'PATCH',
        headers: {
...
export default EditPaymentVoucherModal;