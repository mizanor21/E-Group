...
const DateSelector = ({ control, name, label, watch, required = true }) => {
  ...
  const [calendarDate, setCalendarDate] = useState(new Date()); // Moved this up from the render function
  ...
  useEffect(() => {
    if (watchedDate && isValid(new Date(watchedDate))) {
      ...
      setCalendarDate(new Date(watchedDate)); // Update calendarDate when watchedDate changes
    }
  }, [watchedDate]);
